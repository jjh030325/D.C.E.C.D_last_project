...
void getTable(const char* inputFile) {
	...
	while (1) { // 파일 전체를 읽기 위한 루프문
		if (fread(&ch, 1, 1, output) != 1) break; // 한 글자씩 읽고 끝까지 가면 탈출
		size++;
	...
	printf("현재 파일 위치 : %ld\n", ftell(output)); // 확인용 출력함수(임시 print함수)
	printf("FIEL SIZE : %d\n", size); // 확인용 출력함수(임시 print함수)
...
void checkData() {
	// 변조 체크를 하는 함수
	/*
	* 2개의 데이터를 비교하여 변조된 4곳을 특정한다
	* 압축데이터에서 변조된 것은 변조 하나당 구조체로 구현하여 정리한다
	* 아스키 데이터에서 변조된것은 데이터의 위치를 전역변수에 저장한다
	* 구조체를 이용하여 데이터를 복원할 때 아스키 데이터를 이용하고, 만약 아스키 데이터가 변조되었으면 그것은 제외하고 다른 데이터를 이용하여 복원한다.
	* 
	*/
