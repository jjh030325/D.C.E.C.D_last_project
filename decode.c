...
int main(int argc, char* argv[]) {
	...
	// input 파일에서 데이터 전체를 문자열에 저장
	getTable(inputFile);

	// 그 속에서 변조된 부분 찾음과 동시에 구조체 할당
	checkVariable();

	// 변조된 부분 복원하고 input파일에 복구된 문자열을 덮어쓰기
	dataRestore(inputFile, outputFile);

	// 압축된 문자열을 원본데이터로 복원
	fileRestore(inputFile, outputFile);
	...
	if (Avar_num_count == 0) {
		if (Cvar[Cvar_num_index].row == Cvar[(Cvar_num_index + 1) % 4].row || // 변조된 변수가 같은 행에 있을 경우 조건문이 참
			...
			if (Cvar[Cvar_num_index].column == Cvar[(Cvar_num_index + 1) % 4].column || // 변조된 변수가 같은 열에 있을 경우 조건문이 참
				...
				//높이 값(높이 아스키 데이터)을 이용한 변조 복구 함수
				else {
			...
			// 가로 값(가로 아스키 데이터)을 이용한 변조 복구 함수
			else {
		...
		// 세로 값(세로 아스키 데이터)을 이용한 변조 복구 함수
		else {
